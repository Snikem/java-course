...
public record Animal(
    ...
    public static List<Animal> sortAnimalsByHightASC(List<Animal> list) {
    ...
    {
        return list.stream().filter(it -> it.height < k).sorted(((o1, o2) -> reverseOrder().compare(o1.weight,o2.weight))).findFirst();
    }
    ...
    public static Map<Animal.Type, Integer> weightEachTypeWith(List<Animal> list, int k, int l)
    {
        ...
        for(Type t: Type.values())
        {
            int temp = list.stream().filter(it -> it.age > k && it.age < l).filter(it -> it.type == t).reduce(0,(subtotal,elem) -> subtotal + elem.weight,Integer::sum);
            result.put(t,temp);
